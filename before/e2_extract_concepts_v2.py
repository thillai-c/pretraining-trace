...
Usage:
    ...
    python e2_extract_concepts_v2.py --model olmo2-7b-instruct --training-phase pretraining --test --record_id 30

    # Batch mode: submit all compliant records to Batch API
    python e2_extract_concepts_v2.py --model olmo2-7b-instruct --training-phase pretraining --batch

    # Collect mode: retrieve batch results and save JSON
    python e2_extract_concepts_v2.py --model olmo2-7b-instruct --training-phase pretraining --collect

    # Retry mode: re-run failed records
    python e2_extract_concepts_v2.py --model olmo2-7b-instruct --training-phase pretraining --retry
...
Reference:
    ...
    - Output file: results/{out_dir}/{training_phase}/e2_concepts.json
"""
...
# Subdirectory under results/{out_dir}/ for E1 input and E2 outputs (align with e1_verbatim_trace).
TRAINING_PHASES = ("pretraining", "mid_training", "post_training")


def model_results_root(model_key: str, training_phase: str) -> str:
    """results/{out_dir}/{training_phase}/"""
...
def setup_logger(model_key: str, training_phase: str):
    """Create logger with model- and phase-based log directory.
    Log file: ``logs/{out_dir}/{training_phase}/e2_extract_concepts.log``"""
    out_dir = MODELS[model_key]["out_dir"]
...
def load_e1_results(model_key: str, input_path: str = None,
    ...
    Default path: ``results/{out_dir}/{training_phase}/e1_verbatim_standard.json``
    """
...
def run_test(client, model_key, records, extraction_model, logger, training_phase,
    ...
    test_path = os.path.join(model_results_root(model_key, training_phase),
                             "e2_concepts_test.json")
...
def run_batch(client, model_key, records, extraction_model, logger, training_phase):
    ...
    logger.info(
        "    python e2_extract_concepts_v2.py --model %s --training-phase %s --collect",
        model_key,
        training_phase,
    )
    logger.info("=" * 70)
...
def run_collect(client, model_key, records, extraction_model, batch_id, logger,
    ...
    # Save final output
    output_path = os.path.join(model_results_root(model_key, training_phase),
                               "e2_concepts.json")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Training stage folder under results/{out_dir}/: pretraining, "
             "mid_training, or post_training (default E1 input and E2 outputs)",
    )
...
def main():
    load_dotenv()
    args = parse_args()
    logger = setup_logger(args.model, args.training_phase)
