...
=== v1 버그 ===
v1에서는 get_doc_by_rank(s=0, rank=doc_ix)를 호출했으나,
get_doc_by_rank()의 rank 파라미터는 suffix array 내 rank이지
document index(doc_ix)가 아니다. 따라서 완전히 다른 문서를 가져왔음.

=== v2 수정 전략 ===
1. 각 unique doc_ix에 대해, e1_verbatim_standard_9cases.json에서
   해당 doc_ix를 포함하는 snippet의 snippet_token_ids를 가져온다.
2. find(snippet_token_ids) → segment_by_shard로 suffix array에서 위치를 찾는다.
   (snippet_token_ids는 51~80 tokens로 corpus에서 거의 unique)
3. 각 occurrence에 대해 get_doc_by_rank(s, rank, max_disp_len=small)으로
   doc_ix를 확인한다.
4. target doc_ix와 일치하면, get_doc_by_rank(s, rank, max_disp_len=doc_len)으로
   full document를 가져온다.
